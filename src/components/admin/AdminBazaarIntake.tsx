...
//                 중복 파일은 삭제함.
//   2026-06-23  [수정] 저장 성공 시 강제 상단 스크롤(window.scrollTo top:0) 제거.
//               수정→해당 행을 화면 중앙으로(data-intake-id 타깃), 신규→위치 유지.
// ============================================================================
...
export function AdminBazaarIntake() {
  ...
  const handleFormSuccess = (mode: FormMode) => {
    const editedId = mode?.type === 'edit' ? mode.row.id : null; // ← [2026-06-23] 수정 대상 행 id
    setFormMode(null);
    setFormDirty(false);
    showToast(mode?.type === 'edit' ? '수정되었습니다.' : '접수 등록이 완료되었습니다.');
    void reload();
    // ← [2026-06-23] 기존 강제 상단 스크롤(window.scrollTo top:0) 제거.
    //   수정이면 해당 행을 화면 중앙으로(모달 닫힘 레이아웃 확정 후 2프레임), 신규는 위치 유지.
    if (editedId) {
      requestAnimationFrame(() =>
        requestAnimationFrame(() =>
          document
            .querySelector(`[data-intake-id="${editedId}"]`)
...
function IntakeCard({
  ...
  return (
    <div data-intake-id={row.id} style={{ ...cardBox, opacity: faded ? 0.7 : 1 }}>{/* ← [2026-06-23] 저장 후 중앙 스크롤 타깃 */}
