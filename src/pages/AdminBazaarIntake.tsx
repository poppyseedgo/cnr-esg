...
import { Lightbox } from '@/components/Lightbox'; // ← [2026-06-22] 검수 사진 클릭 확대
...
function IntakeCard({
  ...
  const [lightboxIdx, setLightboxIdx] = useState<number | null>(null); // ← [2026-06-22] 검수 사진 확대
  ...
  // ← [2026-06-22] 확대용 사진 모음: 게시 사진 + 검수 사진(중복 제거)
  ...
  return (
    <div style={{ ...cardBox, opacity: faded ? 0.7 : 1 }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        {/* 썸네일 — 클릭 시 검수/게시 사진 확대 // ← [2026-06-22] */}
        <div
          onClick={() => { if (photos.length) setLightboxIdx(0); }}
          title={photos.length ? '클릭하면 사진 크게 보기' : undefined}
          style={{
            position: 'relative',
            width: 64,
            height: 64,
