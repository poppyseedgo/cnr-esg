      ...
      {rows.length > PER && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: 12 }}>
          <button
            ...
            style={{ padding: '6px 12px', border: '1px solid #e5e7eb', background: '#fff', borderRadius: 8, fontSize: 12, cursor: safePage === 0 ? 'not-allowed' : 'pointer', opacity: safePage === 0 ? 0.5 : 1 }}
          >← 이전</button>
          <span style={{ fontSize: 12, color: '#6b7280' }}>{safePage + 1} / {totalPages}</span>
          <button
            type="button" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={safePage >= totalPages - 1}
            style={{ padding: '6px 12px', border: '1px solid #e5e7eb', background: '#fff', borderRadius: 8, fontSize: 12, cursor: safePage >= totalPages - 1 ? 'not-allowed' : 'pointer', opacity: safePage >= totalPages - 1 ? 0.5 : 1 }}
          >다음 →</button>
        </div>
      )}
